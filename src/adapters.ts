...
export async function createClaudeCompletion(
  ...
  refreshToken: string, stream = false, conversationId?: string, tools?: any[], isNetworking = true, isThinking = false
  ...
  if (stream) {
    const glmStream = await createCompletionStream(glmMessages, refreshToken, model, conversationId, 0, openaiTools, isNetworking, isThinking);
    return convertGLMStreamToClaude(glmStream);
  } else {
    const glmResponse = await createCompletion(glmMessages, refreshToken, model, conversationId, 0, openaiTools, isNetworking, isThinking);
...
export async function createGeminiCompletion(
  ...
  refreshToken: string, stream = false, conversationId?: string, isNetworking = true, isThinking = false
): Promise<any | ReadableStream> {
  const glmMessages = convertGeminiToGLM(contents, systemInstruction);
  if (stream) {
    const glmStream = await createCompletionStream(glmMessages, refreshToken, model, conversationId, 0, undefined, isNetworking, isThinking);
    return convertGLMStreamToGemini(glmStream);
  } else {
    const glmResponse = await createCompletion(glmMessages, refreshToken, model, conversationId, 0, undefined, isNetworking, isThinking);
