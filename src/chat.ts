...
function convertToolMessages(messages: any[]): any[] {
  return messages.map((m: any) => {
    ...
    // 将 assistant 的 tool_calls 转换为自然语言，让模型能看到自己调用过工具的历史，避免重复调用
    if (m.role === "assistant" && Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
      const toolDesc = m.tool_calls.map((tc: any) => {
...
export async function createCompletion(messages: any[], refreshToken: string, model = MODEL_NAME, refConvId = "", retryCount = 0, tools?: any[], isNetworking = true, isThinking = false): Promise<any> {
  return (async () => {
    ...
    let chatMode = '';
    if (isThinking || model.includes('think') || model.includes('zero')) { chatMode = 'zero'; }
    ...
    if (retryCount < MAX_RETRY_COUNT) {
      ...
      return createCompletion(messages, refreshToken, model, refConvId, retryCount + 1, tools, isNetworking, isThinking);
    }
    throw err;
  });
}

export async function createCompletionStream(messages: any[], refreshToken: string, model = MODEL_NAME, refConvId = "", retryCount = 0, tools?: any[], isNetworking = true, isThinking = false): Promise<ReadableStream> {
  return (async () => {
    let processedMessages = convertToolMessages(messages);
    processedMessages = injectToolsPrompt(processedMessages, tools || []);
    const refFileUrls = extractRefFileUrls(processedMessages);
    ...
    if (retryCount < MAX_RETRY_COUNT) {
      console.error(`Stream response error: ${err.stack || err.message}`);
      await sleep(RETRY_DELAY);
      return createCompletionStream(messages, refreshToken, model, refConvId, retryCount + 1, tools, isNetworking, isThinking);
    }
    throw err;
  });
}
